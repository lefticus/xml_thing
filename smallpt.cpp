...
struct RNG {
  ...
  constexpr double operator()() {
    return double(pcg()) / pcg.max();
    //    return double(distribution(pcg, 0, 100000))/100000;
  }
};

struct Vec {      // Usage: time ./smallpt 5000 && xv image.ppm
  double x, y, z; // position, also color (r,g,b)
  constexpr Vec(double x_ = 0, double y_ = 0, double z_ = 0)
      : x{x_}, y{y_}, z{z_} {}
  ...
  constexpr Vec operator*(double b) const { return Vec(x * b, y * b, z * b); }
  constexpr Vec mult(const Vec &b) const {
    return Vec(x * b.x, y * b.y, z * b.z);
  }
  constexpr Vec &norm() {
...
struct Ray {
  Vec o, d;
  constexpr Ray(Vec o_, Vec d_) : o(o_), d(d_) {}
};
...
struct Sphere {
  ...
  constexpr Sphere(double rad_, Vec p_, Vec e_, Vec c_, Refl_t refl_)
...
constexpr int toInt(double x) { return int(std::lerp(0.0, 255.0, x)); }
bool intersect(const Ray &r, double &t, int &id) {
  double d;
  constexpr double inf = 1e20;
  t = inf;

  for (int i = spheres.size(); i--;) {
...
Vec radiance(const Ray &r, int depth, RNG &rng) {
  ...
  if (obj.refl == DIFF) { // Ideal DIFFUSE reflection
    ...
    return obj.e +
           f.mult(radiance(Ray(x, r.d - n * 2 * n.dot(r.d)), depth, rng));
  ...
  return obj.e +
         f.mult(depth > 2
                    ? (rng() < P ? // Russian roulette
                           radiance(reflRay, depth, rng) * RP
                                 : radiance(Ray(x, tdir), depth, rng) * TP)
                    : radiance(reflRay, depth, rng) * Re +
                          radiance(Ray(x, tdir), depth, rng) * Tr);
}
int main(int argc, char *argv[]) {
  constexpr int w = 320;
  constexpr int h = 240;
  const int samps = argc == 2 ? atoi(argv[1]) / 4 : 1; // # samples
  constexpr Ray cam(Vec(50, 52, 295.6),
      ...
      for (int sy = 0, i = (h - y - 1) * w + x; sy < 2;
        ...
        for (int sx = 0; sx < 2; sx++, r = Vec()) { // 2x2 subpixel cols
          for (int s = 0; s < samps; s++) {
            ...
            r = r +
                radiance(Ray(cam.o + d * 140, d.norm()), 0, rng) * (1. / samps);
          } // Camera rays are pushed ^^^^^ forward to start in interior
          c[i] =
              c[i] + Vec(std::clamp(r.x, 0.0, 1.0), std::clamp(r.y, 0.0, 1.0),
