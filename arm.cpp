...
struct Strongly_Typed
{
  ...
  constexpr auto operator&(const Type rhs) const noexcept
  {
    return m_val & rhs;
  }

  friend constexpr auto operator&(const Type lhs, const CRTP rhs) noexcept
  {
    return lhs & rhs.m_val;
  }

  constexpr bool bit_set(const Type bit) const noexcept
  {
    return (m_val & (1 << bit));
  }
...
struct Instruction : Strongly_Typed<std::uint32_t, Instruction>
{
  ...
  [[nodiscard]] constexpr Condition get_condition() const noexcept
  {
    return static_cast<Condition>(0b1111 & (m_val >> 28));
  }
...
struct Data_Processing : Strongly_Typed<std::uint32_t, Data_Processing>
{
  ...
  [[nodiscard]] constexpr auto operand_2() const noexcept
  {
    return 0b1111'1111'1111 & m_val;
  }

  [[nodiscard]] constexpr auto operand_2_register() const noexcept
  {
    return 0b1111 & m_val;
  }

  [[nodiscard]] constexpr bool operand_2_immediate_shift() const noexcept
  {
    return !bit_set(4);
  }

  [[nodiscard]] constexpr auto operand_2_shift_register() const noexcept
  {
    return operand_2() >> 8;
  }

  [[nodiscard]] constexpr auto operand_2_shift_amount() const noexcept
  {
    return operand_2() >> 7;
  }

  [[nodiscard]] constexpr auto operand_2_shift_type() const noexcept
  {
    return static_cast<Shift_Type>((operand_2() >> 1) & 0b11);
  }
  ...
  constexpr auto destination_register() const noexcept
  {
    return 0b1111 & (m_val >> 12);
  }

  [[nodiscard]] constexpr auto operand_1_register() const noexcept
  {
    return 0b1111 & (m_val >> 16);
  }

  [[nodiscard]] constexpr bool set_condition_code() const noexcept
  {
    return bit_set(20);
  }

  [[nodiscard]] constexpr bool immediate_operand() const noexcept
  {
    return bit_set(25);
  }
};
...
[[nodiscard]] constexpr auto get_lookup_table() noexcept
{
  ...
  auto bitcount = [](const auto &tuple) {
  ...
  constexpr auto swap_tuples = [](auto &lhs, auto &rhs) noexcept
  {
    auto old = lhs;
  ...
  for (auto itr = table.begin(); itr != end(table); ++itr) {
    swap_tuples(*itr,
      *std::min_element(itr, end(table), [bitcount](const auto &lhs, const auto &rhs) {
...
struct System
{
  ...
  [[nodiscard]] constexpr const auto &PC() const noexcept
  {
    return registers[15];
  }
  ...
  [[nodiscard]] constexpr std::pair<bool, std::uint32_t> shift_register(const bool c_flag, const Shift_Type type, std::uint32_t shift_amount, std::uint32_t value) const noexcept
  {
    switch (type) {
  ...
  constexpr void data_processing(const Data_Processing val) noexcept
  {
    ...
    const auto update_logical_flags = [=,&destination](const bool write, const auto result) {
    ...
    const auto arithmetic =
      [=,&destination, carry = c_flag(), first_operand = static_cast<std::uint64_t>(first_operand)](const bool write, const auto op)
    {
    ...
    // Logical Operations
    case OpCode::AND:
      update_logical_flags(true, first_operand & second_operand);
      break;
    case OpCode::EOR:
      update_logical_flags(true, first_operand ^ second_operand);
      break;
    case OpCode::TST:
      update_logical_flags(false, first_operand & second_operand);
      break;
    case OpCode::TEQ:
      update_logical_flags(false, first_operand ^ second_operand);
      break;
    ...
    case OpCode::SUB:
      arithmetic(true, [](const auto op_1, const auto op_2, const auto) { return op_1 - op_2; });
      break;
    case OpCode::RSB:
      arithmetic(true, [](const auto op_1, const auto op_2, const auto) { return op_2 - op_1; });
      break;
    case OpCode::ADD:
      arithmetic(true, [](const auto op_1, const auto op_2, const auto) { return op_1 + op_2; });
      break;
    case OpCode::ADC:
      arithmetic(true, [](const auto op_1, const auto op_2, const auto c) { return op_1 + op_2 + c; });
      break;
    case OpCode::SBC:
      arithmetic(true, [](const auto op_1, const auto op_2, const auto c) { return op_1 - op_2 + c - 1; });
      break;
    case OpCode::RSC:
      arithmetic(true, [](const auto op_1, const auto op_2, const auto c) { return op_2 - op_1 + c - 1; });
      break;
    case OpCode::CMP:
      arithmetic(false, [](const auto op_1, const auto op_2, const auto) { return op_1 - op_2; });
      break;
    case OpCode::CMN:
      arithmetic(false, [](const auto op_1, const auto op_2, const auto) { return op_1 + op_2; });
      break;
  ...
  constexpr bool n_flag() const noexcept
  {
    return CSPR & n_bit;
  }
  constexpr void n_flag(const bool val) noexcept
  {
    set_or_clear_bit(CSPR, n_bit, val);
  }
  constexpr bool z_flag() const noexcept
  {
    return CSPR & z_bit;
  }
  constexpr void z_flag(const bool val) noexcept
  {
    set_or_clear_bit(CSPR, z_bit, val);
  }
  constexpr bool c_flag() const noexcept
  {
    return CSPR & c_bit;
  }

  constexpr void c_flag(const bool val) noexcept
  {
    set_or_clear_bit(CSPR, c_bit, val);
  }

  constexpr bool v_flag() const noexcept
  {
    return CSPR & v_bit;
  }

  constexpr void v_flag(const bool val) noexcept
  {
    set_or_clear_bit(CSPR, v_bit, val);
  }
  ...
  constexpr void process(const Instruction instruction) noexcept
  {
    ...
    if (check_condition(instruction)) {
      ...
      case Instruction_Type::Multiply:
        ...
        break;
      case Instruction_Type::Long_Multiply:
        long_multiply(instruction);
        break;
      case Instruction_Type::Swap:
        swap(instruction);
        break;
      case Instruction_Type::Load_Store_Byte_Word:
        load_store_byte_word(instruction);
        break;
      case Instruction_Type::Load_Store_Multiple:
        load_store_multiple(instruction);
        break;
      case Instruction_Type::Half_Word_Transfer_Immediate_Offset:
        half_word_transfer_immediate_offset(instruction);
        break;
      case Instruction_Type::Half_Word_Transfer_Register_Offset:
        half_word_transfer_register_offset(instruction);
        break;
      case Instruction_Type::Branch:
        branch(instruction);
        break;
      case Instruction_Type::Branch_Exchange:
        branch_exchange(instruction);
        break;
      case Instruction_Type::Coprocessor_Data_Transfer:
        coprocessor_data_transfer(instruction);
        break;
      case Instruction_Type::Coprocessor_Data_Operation:
        coprocessor_data_operation(instruction);
        break;
      case Instruction_Type::Coprocessor_Register_Transfer:
        coprocessor_register_transfer(instruction);
        break;
      case Instruction_Type::Software_Interrupt:
        software_interrupt(instruction);
        break;
      case Instruction_Type::Unhandled_Opcode:
        break;
      }
    }
  }
...
template<typename... T>
constexpr auto run_instruction(T... instruction)
{
  System system;
...
constexpr auto systest6 = run_instruction(

 Instruction{0xe2800001}, // add r0, r0, #1
...
//static_assert(systest6.registers[3] == 1 - (9 >> 2));
...
{
  Instruction instruction{ static_cast<std::uint32_t>(argc) };
  System sys;
  sys.process(instruction);
}
